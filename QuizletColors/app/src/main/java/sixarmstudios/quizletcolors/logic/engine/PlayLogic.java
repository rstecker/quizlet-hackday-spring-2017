...
public class PlayLogic implements IPlayLogic {
    ...
    private @NonNull QCGameMessage handleMove(@NonNull QCMember player, @NonNull QCMember asker, @NonNull String answer) {
    ...
    private @NonNull QCGameMessage handleCorrectMove(@NonNull QCMember player, @NonNull QCMember asker, @NonNull String answer, @NonNull String question) {
    ...
    private @NonNull QCGameMessage handleBadMove(@NonNull QCMember player, @NonNull QCMember asker, @NonNull String providedAnswer, @NonNull String answeredQuestion,  @NonNull String correctAnswer) {
        ...
        QCGameMessage msg = mEngine.generateBasePlayMessage(action)
                .setInfoForBadMove(providedAnswer, answeredQuestion, correctAnswer, correctQuestion, askersOfAnswer)
                .setReactionPlayer(mEngine.findMemberByUsername(player.username()), askersOfAnswer.size() > 0 ? QCMember.Reaction.WRONG_USER : QCMember.Reaction.WRONG_CHOICE)
                .setReactionPlayer(mEngine.findMemberByUsername(asker.username()), QCMember.Reaction.RECEIVED_BAD_ANSWER);
